...
7. **Добавление Alias:** В файле конфига прописали алаясы для более удобного обращения к файлам через псевдонимы:
```
    server {
    listen 443 ssl;
    server_name localhost;
...
8. **Тестирование работы:** После завершения настройки Docker Compose и Nginx была запущена сборка контейнеров, и оба
   сайта были в ручную протестированы.

## Вывод:

В результате выполнения лабораторной работы был настроен веб-сервер Nginx с поддержкой нескольких доменов и SSL, а также
настроен редирект с HTTP на HTTPS для обоих пет-проектов. Так же были настроены псевдонимы путей до файлов Alias.
